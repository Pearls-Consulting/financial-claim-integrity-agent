...
STAGES: list[Stage] = [
    ...
    Stage(
        id="three_way_match",
        order=3,
        title_en="Acceptance & three-way match",
        title_ar="الاستلام والمطابقة الثلاثية",
        desc_en="One acceptance document per contract kind — goods receipt for goods, Certificate of Completion for works — matched against contract/BoQ and invoice: billed only what was accepted, within contracted quantities.",
        desc_ar="مستند استلام واحد بحسب نوع العقد — إيصال الاستلام للتوريد ومحضر الإنجاز للأعمال — يُطابق مع العقد وجدول الكميات والفاتورة: لا فوترة إلا لما تم استلامه وضمن الكميات التعاقدية.",
        rulepack="three_way.yaml",
    ),
    Stage(
        id="final_check",
        order=4,
        title_en="Final check — penalties & delay",
        title_ar="الفحص النهائي — الغرامات والتأخير",
        desc_en="Penalty record cross-checked against the acceptance document and the contract dates: declared delay, inferred delay, penalties due.",
        desc_ar="مطابقة سجل الغرامات مع مستند الاستلام وتواريخ العقد: التأخير المصرّح به، والتأخير المستنتج من التواريخ، والغرامات المستحقة.",
        rulepack="final_check.yaml",
    ),
    Stage(
        id="prefinance",
        order=5,
